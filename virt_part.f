...
c----------------------------------------------------------------------
c   Subroutine for generating virtual particles
c   including mirror, dynamic, dummy and repulsive
...
c     ocn     : number of over generation                          [out]
...
c      common nvirt
      ...
      if (vp_input) then          
        ...
        open(10,file="../data/xv_vp.dat")
        open(20,file="../data/state_vp.dat")
        open(30,file="../data/other_vp.dat")            
        read(10,*) nvirt
        do j = 1, nvirt   
          i = ntotal + j      
          read(10,*)im, (x(d, i),d = 1, dim), (vx(d, i),d = 1, dim)                     
          read(20,*)im, mass(i), rho(i), p(i), u(i)        
          read(30,*)im, itype(i), hsml(i), mother(i)                            
        enddo  
        close(10)
        close(20) 
        close(30) 
...
c   two bottom corners           
        do j = 1,2
           ...
           if(dps.le.scale_k*hsml(i).and.dps.gt.1e-8)then
               nvirt = nvirt+1
               do d =1,dim
     ...
     &   nnp,ntotal,nvirt,x,vx,mother,ocn)
        ...
        do i=ntotal+1,ntotal+nvirt
            ...
            hsml(i)= 1.3*dx
            p(i)=p(mother(i))
c            if (i.eq.ntotal+100) print *,p(i)
            rho(i)=rho(mother(i))
            u(i)=u(mother(i))
...
c     Monaghan type virtual particle(repulsive force)
      if(ex_force) then
c     Lower side              
        do i = 1, 2*mp+1
...
c    symmetric to centerline 
       if(geometry)then   
        ...
        do i = 1, qp*4
          nwall = nwall + 2
          x(1,ntotal+nvirt+nwall-1) = x_mingeom+(np-qp)*dx+(i+1)*dx/4
